window.READING_SHELF_BOOKS = [
    {
        "bookId": "CB_3zo18G19F1tP71L71J6MaGlu",
        "title": "思考,快与慢",
        "coverTitle": "思考,快与慢",
        "author": "丹尼尔•卡尼曼",
        "publisher": "CTCCN",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_3zo18G19F1tP71L71J6MaGlu_parsecover",
        "lastReadDate": "2026-05-01",
        "readingTime": 3236,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1sCF7sFAB7vQ6Wi6YI",
        "title": "工作、消费主义和新穷人",
        "coverTitle": "工作、消费主义和新穷人",
        "author": "(英)齐格蒙特·鲍曼",
        "publisher": "上海社会科学院出版社有限公司",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_BDb6tD6tM09i6Ty6U4_parsecover",
        "lastReadDate": "2022-04-09",
        "readingTime": 2422,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22973166",
        "title": "今日简史：人类命运大议题",
        "coverTitle": "今日简史：人类命运大议题",
        "author": "尤瓦尔·赫拉利",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/32/YueWen_22973166/t9_YueWen_22973166.jpg",
        "lastReadDate": "2026-01-16",
        "readingTime": 140,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_3Ol0vz0xKFQC6dA6cR",
        "title": "我们仨",
        "coverTitle": "我们仨",
        "author": "杨绛",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://weread-1258476243.file.myqcloud.com/app/assets/bookcover/book_cover_default_imported_02.png",
        "lastReadDate": "2022-12-11",
        "readingTime": 759,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_9tsDtcDsu3Pc6bo6cR",
        "title": "少有人走的路(1-8全套)",
        "coverTitle": "少有人走的路(1-8全套)",
        "author": "斯科特·派克",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_9tsDtcDsu3Pc6bo6cR_parsecover",
        "lastReadDate": "2024-11-22",
        "readingTime": 3965,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1LI3sC3tC6Gr70H71J9Yj7af",
        "title": "与神对话(全五卷)(“与神系列”终极完整版。完整收录《与神对话·全三卷》及《与神为友》《与神合一》)(套装共5册)",
        "coverTitle": "与神对话(全五卷)(“与神系列”终极完整版。完整收录《与神对话·全三卷》及《与神为友》《与神合一》)(套装共5册)",
        "author": "尼尔·唐纳德·沃尔什",
        "publisher": "江西人民出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_1LI3sC3tC6Gr70H71J9Yj7af_parsecover",
        "lastReadDate": "2025-12-26",
        "readingTime": 2130,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_0PmBCgBFg15f71971J3798Uk",
        "title": "巴菲特之道",
        "coverTitle": "巴菲特之道",
        "author": "（美）罗伯特·哈格斯特朗（Robert G. Hagstrom） 著",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_0PmBCgBFg15f71971J3798Uk_parsecover",
        "lastReadDate": "2026-01-11",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "27689995",
        "title": "查拉图斯特拉如是说（果麦经典）",
        "coverTitle": "查拉图斯特拉如是说（果麦经典）",
        "author": "尼采",
        "publisher": "上海文化出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：钱春绮",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/7/YueWen_27689995/t9_YueWen_27689995.jpg",
        "lastReadDate": "2025-10-11",
        "readingTime": 6220,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300132901",
        "title": "清算已毕：波伏瓦自传",
        "coverTitle": "清算已毕：波伏瓦自传",
        "author": "[法]西蒙娜·德·波伏瓦",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/85/cpplatform_aeadwyzdkvsbhkuashe48f/t9_cpplatform_aeadwyzdkvsbhkuashe48f1738918192.jpg",
        "lastReadDate": "2025-06-01",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "244004",
        "title": "冬牧场",
        "coverTitle": "冬牧场",
        "author": "李娟",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/22/yuewen_244004/t9_yuewen_2440041690201500.jpg",
        "lastReadDate": "2025-04-22",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25982805",
        "title": "洛克菲勒写给儿子的38封信",
        "coverTitle": "洛克菲勒写给儿子的38封信",
        "author": "洛克菲勒",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/65/YueWen_25982805/t9_YueWen_25982805.jpg",
        "lastReadDate": "2025-12-01",
        "readingTime": 471,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "833591",
        "title": "富兰克林传",
        "coverTitle": "富兰克林传",
        "author": "沃尔特·艾萨克森",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/25/YueWen_833591/t9_YueWen_833591.jpg",
        "lastReadDate": "2025-04-10",
        "readingTime": 7417,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_9AXH1lH1j2cO6c86cR",
        "title": "程序员修炼之道：通向务实的最高境界（第2版）",
        "coverTitle": "程序员修炼之道：通向务实的最高境界（第2版）",
        "author": "(美）David Thomas（大卫·托马斯），Andrew Hunt（安德鲁·亨特）",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_9AXH1lH1j2cO6c86cR_parsecover",
        "lastReadDate": "2025-09-25",
        "readingTime": 2060,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "848673",
        "title": "当下的力量（白金版）",
        "coverTitle": "当下的力量（白金版）",
        "author": "埃克哈特·托利",
        "publisher": "中信出版集团",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：曹植",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/2/YueWen_848673/t9_YueWen_848673.jpg",
        "lastReadDate": "2025-04-10",
        "readingTime": 2258,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_6PoELAELv1UT71B71JFD08tB",
        "title": "超越百岁_长寿的科学与艺术_美_彼得阿提亚_比尔吉福德",
        "coverTitle": "超越百岁_长寿的科学与艺术_美_彼得阿提亚_比尔吉福德",
        "author": "【美】彼得·阿提亚；比尔·吉福德",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_F9W0dO0f63oU6oI6ooGWi3P2_parsecover",
        "lastReadDate": "2026-05-14",
        "readingTime": 1256,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_ENbBFXBFg15f71971JBwh2GX",
        "title": "巴菲特致股东的信_权威修订版",
        "coverTitle": "巴菲特致股东的信_权威修订版",
        "author": "查理·芒格(Charlie Munger)",
        "publisher": "北京联合出版公司",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_ENbBFXBFg15f71971JBwh2GX_parsecover",
        "lastReadDate": "2026-05-11",
        "readingTime": 1928,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25184495",
        "title": "当我谈跑步时，我谈些什么",
        "coverTitle": "当我谈跑步时，我谈些什么",
        "author": "[日]村上春树",
        "publisher": "南海出版公司",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：施小炜",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/10/yuewen_25184495/t9_yuewen_251844951740049944.jpg",
        "lastReadDate": "2025-09-24",
        "readingTime": 1650,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_6kXGTNGUZAuO6ym6xA01P9iV",
        "title": "正念的奇迹",
        "coverTitle": "正念\n的奇迹",
        "author": "一行禅师",
        "publisher": "中央编译出版社",
        "status": "在读",
        "progress": 30,
        "notes": 2,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_6kXGTNGUZAuO6ym6xA01P9iV_parsecover",
        "lastReadDate": "2026-06-26",
        "readingTime": 7260,
        "detail": {
            "summary": "作者用平淡温馨的布道语气，充满博爱地传递正念的真谛。教会了我呼吸——无意识憋气源于Ni过度运转的身体反应，觉察呼吸就是松开Ni的闸。正念的目的不是保持正念，而是觉察到自己的念头漂移，不评判地拉回。",
            "thesis": "正念的本质不是维持专注状态，而是在觉察到分心时不带评判地回到当下。",
            "reader": "INTJ — 憋气是Ni在憋，呼吸自由是Se在接管。",
            "distance": "零距离 — 正在改变我的身体反应模式",
            "originalFeeling": "一行禅师的文字没有宗教感，却有一种奇怪的接近妈妈读睡前故事的安心感。他不是在教一个技巧，是在告诉你活着可以这样简单。",
            "collisions": [],
            "scanDate": "2026-06-26",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300070952",
        "title": "看见孩子",
        "coverTitle": "看见孩子",
        "author": "贝姬·肯尼迪",
        "publisher": "中信出版集团",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：美同",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/51/cpplatform_59zd8bsvew2haapnupon1r/t9_cpplatform_59zd8bsvew2haapnupon1r1694596495.jpg",
        "lastReadDate": "2025-04-01",
        "readingTime": 980,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_9R7D25D1X1l371F71J4EwFbH",
        "title": "未来简史：从智人到神人",
        "coverTitle": "未来简史：从智人到神人",
        "author": "[以] 尤瓦尔·赫拉利",
        "publisher": "北京：中信出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_9R7D25D1X1l371F71J4EwFbH_parsecover",
        "lastReadDate": "2026-01-23",
        "readingTime": 6371,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_5M04QC4OoEpR6yD6xA2wj3wZ",
        "title": "反抗「平庸之恶」_汉娜阿伦特_上海人民出版社2014.epub",
        "coverTitle": "反抗「平庸之恶」_汉娜阿伦特_上海人民出版社2014.epub",
        "author": "赵 鸿威",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_EJ033C33GDVF6i56gY4Tx3nb_parsecover",
        "lastReadDate": "2025-10-03",
        "readingTime": 441,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "917534",
        "title": "刀锋（果麦经典）",
        "coverTitle": "刀锋（果麦经典）",
        "author": "毛姆",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/534/917534/t9_917534.jpg",
        "lastReadDate": "2025-09-24",
        "readingTime": 564,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_61C2YU2Yo2Da6kM6kb3zd9qW",
        "title": "effective java 3rd 中文版",
        "coverTitle": "effective java 3rd 中文版",
        "author": "wizardforcel",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_9C65CM5FfAZa6i66gw4LB7fr_parsecover",
        "lastReadDate": "2025-04-10",
        "readingTime": 588,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "33819542",
        "title": "瞧，这个人（果麦经典）",
        "coverTitle": "瞧，这个人（果麦经典）",
        "author": "尼采",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/35/YueWen_33819542/t9_YueWen_33819542.jpg",
        "lastReadDate": "2025-09-17",
        "readingTime": 6495,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GZ62a02Yo4bm70Q71JGTOALT",
        "title": "千面英雄",
        "coverTitle": "千面英雄",
        "author": "约瑟夫•坎贝尔（Joseph Campbell)",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_2SJCyhCv4GrM6jk6kfGZOAod_parsecover",
        "lastReadDate": "2026-05-01",
        "readingTime": 2847,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300023500",
        "title": "查拉图斯特拉如是说",
        "coverTitle": "查拉图斯特拉如是说",
        "author": "尼采",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/0/3300023500/t9_3300023500.jpg",
        "lastReadDate": "2025-10-13",
        "readingTime": 436,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25306870",
        "title": "能力陷阱",
        "coverTitle": "能力陷阱",
        "author": "埃米尼亚·伊贝拉",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/870/25306870/t9_25306870.jpg",
        "lastReadDate": "2025-09-12",
        "readingTime": 1714,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "30915661",
        "title": "走近费曼丛书·费曼讲物理：入门",
        "coverTitle": "走近费曼丛书·费曼讲物理：入门",
        "author": "理查德·费曼",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/661/30915661/t9_30915661.jpg",
        "lastReadDate": "2025-11-06",
        "readingTime": 1546,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300102197",
        "title": "不纠结的哲学",
        "coverTitle": "不纠结的哲学",
        "author": "[古罗马]塞涅卡 [古罗马]爱比克泰德 [古罗马]奥勒留",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/20/cpplatform_r9hbbznuvf4bp9hp3t86ob/t9_cpplatform_r9hbbznuvf4bp9hp3t86ob1717474265.jpg",
        "lastReadDate": "2026-02-01",
        "readingTime": 190,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_7Pj0YR0YS2YH6c96cR",
        "title": "原则",
        "coverTitle": "原则",
        "author": "[美]瑞·达利欧",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_7Pj0YR0YS2YH6c96cR_parsecover",
        "lastReadDate": "2025-04-10",
        "readingTime": 545,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_8lv36x35y6nV6zt71JBz8Eke",
        "title": "维特根斯坦传：天才之为责任",
        "coverTitle": "维特根斯坦传：天才之为责任",
        "author": "【英】瑞·蒙克, 王宇光, ePUBw.COM",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 4,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_2pA5t65vMF9d6xl6x63fS6E1_parsecover",
        "lastReadDate": "2026-05-10",
        "readingTime": 2221,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "生活方式 x 语言游戏 — 想锻炼慢思考，不如直接选择一种简单的生活方式。用旁观视角体会当下，少用回顾和展望框定自己。目标不要遥不可及，才能真正让那几棵树开出分叉。",
                "维特根斯坦的生活方式决定语言游戏 x 纳瓦尔的减法哲学 x 斯多葛的沉思录 — 三者共享同一洞见：摆脱自我意识的过度建构，回到生活本身的语言游戏中。",
                "维特根斯坦 x 清晰思考的HALT法则 — HALT是硬性的减速带，维特根斯坦是软性的生活方式转向。前者治标，后者治本。",
                "名字不是预言，是你活出来的。卜斌不是被\"斌\"这个标签决定，而是用三十年活出了\"文\"和\"武\"两面之后，才第一次真正读懂了自己的名字。维特根斯坦说：语言游戏由生活方式决定，不是反过来。"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "854928",
        "title": "枪炮、病菌与钢铁：人类社会的命运（修订版）",
        "coverTitle": "枪炮、病菌与钢铁：人类社会的命运（修订版）",
        "author": "贾雷德·戴蒙德",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/44/YueWen_854928/t9_YueWen_854928.jpg",
        "lastReadDate": "2025-10-14",
        "readingTime": 709,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "853116",
        "title": "杀死一只知更鸟（同名电影原著）",
        "coverTitle": "杀死一只知更鸟（同名电影原著）",
        "author": "哈珀·李",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/32/YueWen_853116/t9_YueWen_853116.jpg",
        "lastReadDate": "2025-12-14",
        "readingTime": 5488,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_Ah6EUSEUDABP6aR6YI",
        "title": "爱德华的奇妙之旅",
        "coverTitle": "爱德华的奇妙之旅",
        "author": "[美]凯特·迪卡米洛",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_Ah6EUSEUDABP6aR6YI_parsecover",
        "lastReadDate": "2025-03-20",
        "readingTime": 7,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_9MyEMeELv1UT71B71J6dN2H7",
        "title": "存在主义心理治疗",
        "coverTitle": "存在主义心理治疗",
        "author": "欧文·D·亚隆",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_E4OAXYAXz3Ye71M71IDWW2iy_parsecover",
        "lastReadDate": "2026-01-21",
        "readingTime": 191,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "24965201",
        "title": "平凡的世界（全三册）",
        "coverTitle": "平凡的世界（全三册）",
        "author": "路遥",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/61/yuewen_24965201/t9_yuewen_249652011682413401.jpg",
        "lastReadDate": "2025-09-08",
        "readingTime": 88,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_0DLErjEt52cO70u71J0To019",
        "title": "《卡耐基：语言的突破》",
        "coverTitle": "《卡耐基：语言的突破》",
        "author": "Unknown",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_0DLErjEt52cO70u71J0To019_parsecover",
        "lastReadDate": "2026-01-11",
        "readingTime": 1475,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_5p1BcFBeY6Ck6wA6xA2HwBlb",
        "title": "穷查理宝典：查理·芒格智慧箴言录",
        "coverTitle": "穷查理宝典：查理·芒格智慧箴言录",
        "author": "查理·芒格",
        "publisher": "中信出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_CqmDqiDqo2a36i36gp6yv0RP_parsecover",
        "lastReadDate": "2025-10-13",
        "readingTime": 2458,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1LI0sO0tBFQC6dA6cR",
        "title": "肠子的小心思",
        "coverTitle": "肠子的小心思",
        "author": "【德】朱莉娅•恩德斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_1LI0sO0tBFQC6dA6cR_parsecover",
        "lastReadDate": "2026-05-11",
        "readingTime": 4556,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_CQ6EPyEQ41UT71B71JAz10vL",
        "title": "1_终身成长：重新定义成功的思维模式 = Mindset The New Psychology of Success ([美] 卡罗尔·德韦克 (Carol S. Dweck) 著；楚祎楠 译) (1)",
        "coverTitle": "1_终身成长：重新定义成功的思维模式 = Mindset The New Psychology of Success ([美] 卡罗尔·德韦克 (Carol S. Dweck) 著；楚祎楠 译) (1)",
        "author": "[美] 卡罗尔·德韦克",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_GMA1J01JmCwq71M71FDuA9M8_parsecover",
        "lastReadDate": "2026-01-21",
        "readingTime": 129,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GdF86j84H3Pc6Xg6YI",
        "title": "社会心理学",
        "coverTitle": "社会心理学",
        "author": "[美]戴维·迈尔斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_6AYFcsFdn1u76Qt6QK_parsecover",
        "lastReadDate": "2025-09-10",
        "readingTime": 2,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_AUvB65B7OAaD6zF6xACjPDkj",
        "title": "国家为什么会失败(中英对照)",
        "coverTitle": "国家为什么会失败(中英对照)",
        "author": "德隆·阿西莫格鲁",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_9nA0w00yK8Rm6iQ6gk8JmD8O_parsecover",
        "lastReadDate": "2025-10-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GhOEIkEHm1UT71B71J9IQGva",
        "title": "wordbuilder",
        "coverTitle": "wordbuilder",
        "author": "INTSIG",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_GhOEIkEHm1UT71B71J9IQGva_parsecover",
        "lastReadDate": "2026-02-09",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_7TcEfYEge7eq6zY71J5ZX6u9",
        "title": "掌中花园_14093999",
        "coverTitle": "掌中花园_14093999",
        "author": "未知作者",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_3qfGMvGNQ4Zw6i46gs17Y7V9_parsecover",
        "lastReadDate": "2025-10-28",
        "readingTime": 1161,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25411686",
        "title": "悲剧的诞生",
        "coverTitle": "悲剧的诞生",
        "author": "尼采",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/686/25411686/t9_25411686.jpg",
        "lastReadDate": "2025-09-16",
        "readingTime": 166,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300024931",
        "title": "小米创业思考",
        "coverTitle": "小米创业思考",
        "author": "雷军 徐洁云",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/31/3300024931/t9_3300024931.jpg",
        "lastReadDate": "2025-12-26",
        "readingTime": 4280,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_FZR37J35y6nV6zt71J8XhCZc",
        "title": "维特根斯坦精选集（全五册）（20世纪哲学家维特根斯坦代表作精选合集。通过维特根斯坦精选集，读懂哲学。）",
        "coverTitle": "维特根斯坦精选集（全五册）（20世纪哲学家维特根斯坦代表作精选合集。通过维特根斯坦精选集，读懂哲学。）",
        "author": "[奥]路德维希•维特根斯坦",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_Bzu4wE4yKEae6rg6sx2VJ9nZ_parsecover",
        "lastReadDate": "2025-11-13",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GtpER3EQ41UT71B71J8DxGoh",
        "title": "爱的艺术",
        "coverTitle": "爱的艺术",
        "author": "弗洛姆",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_GtpER3EQ41UT71B71J8DxGoh_parsecover",
        "lastReadDate": "2026-05-10",
        "readingTime": 517,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300130748",
        "title": "超级沟通者",
        "coverTitle": "超级沟通者",
        "author": "[美]查尔斯·都希格",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/68/cpplatform_1m2nbuds51dnyk2fz6thcr/t9_cpplatform_1m2nbuds51dnyk2fz6thcr1736754494.jpg",
        "lastReadDate": "2025-09-12",
        "readingTime": 755,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25615385",
        "title": "被讨厌的勇气：“自我启发之父”阿德勒的哲学课",
        "coverTitle": "被讨厌的勇气：“自我启发之父”阿德勒的哲学课",
        "author": "岸见一郎 古贺史健",
        "publisher": "机械工业出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：渠海霞",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/385/25615385/t9_25615385.jpg",
        "lastReadDate": "2025-09-25",
        "readingTime": 1710,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "41452789",
        "title": "六项精进",
        "coverTitle": "六项精进",
        "author": "稻盛和夫",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/789/41452789/t9_41452789.jpg",
        "lastReadDate": "2025-10-17",
        "readingTime": 106,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1juBzmC2uAmA6yo6xACNi73P",
        "title": "早上一分钟，改变一整天",
        "coverTitle": "早上一分钟，改变一整天",
        "author": "【韩】柳韩彬",
        "publisher": "国文出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_23bH2dH0m4lz6yF6wwB6l5OA_parsecover",
        "lastReadDate": "2025-10-14",
        "readingTime": 869,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_6Y6EjyEkn0EC71M71J2ea4g1",
        "title": "优秀到不能被忽视",
        "coverTitle": "优秀到不能被忽视",
        "author": "（美）纽波特",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_6Y6EjyEkn0EC71M71J2ea4g1_parsecover",
        "lastReadDate": "2026-01-22",
        "readingTime": 761,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "674048",
        "title": "邓小平时代",
        "coverTitle": "邓小平时代",
        "author": "傅高义",
        "publisher": "生活·读书·新知三联书店",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：冯克利",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/48/674048/t9_674048.jpg",
        "lastReadDate": "2025-05-13",
        "readingTime": 63273,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "914697",
        "title": "了凡四训",
        "coverTitle": "了凡四训",
        "author": "[明]袁了凡",
        "publisher": "三秦出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：费勇",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/28/YueWen_914697/t9_YueWen_914697.jpg",
        "lastReadDate": "2026-01-14",
        "readingTime": 3570,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_DeG2OS2MN0Qf6xJ6xAF1U5I5",
        "title": "蛤蟆先生去看心理医生",
        "coverTitle": "蛤蟆先生",
        "author": "might all",
        "publisher": "天津人民出版社",
        "status": "已读",
        "progress": 100,
        "notes": 1,
        "edition": "心理成长",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "截图 + 旧读书笔记",
        "cover": "https://weread-1258476243.file.myqcloud.com/app/assets/bookcover/book_cover_default_imported_02.png",
        "lastReadDate": "2025-08-10",
        "readingTime": 917,
        "detail": {
            "summary": "旧博客已有读书笔记，书架里先作为待迁移条目保留。",
            "thesis": "待迁移",
            "reader": "待迁移",
            "distance": "待迁移",
            "collisions": [
                "旧读书笔记待迁入 Obsidian 快照"
            ]
        }
    },
    {
        "bookId": "695233",
        "title": "三体全集（全三册）",
        "coverTitle": "三体全集（全三册）",
        "author": "刘慈欣",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/80/yuewen_695233/t9_yuewen_6952331740758482.jpg",
        "lastReadDate": "2025-11-19",
        "readingTime": 785,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_5DyEPmEQ41UT71B71JCKA1CP",
        "title": "深度工作",
        "coverTitle": "深度工作",
        "author": "(美)卡尔·纽波特 著；宋伟 译",
        "publisher": "南昌：江西人民出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_Ca9EufEtLFbu71G71HBV0AX6_parsecover",
        "lastReadDate": "2026-01-17",
        "readingTime": 1162,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "34357450",
        "title": "变形记",
        "coverTitle": "变形记",
        "author": "卡夫卡",
        "publisher": "浙江人民出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：姬健梅",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/450/34357450/t9_34357450.jpg",
        "lastReadDate": "2025-09-15",
        "readingTime": 7425,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_AMdEUdEUDABP6aR6YI",
        "title": "失落的卫星 : 深入中亚大陆的旅程（“全球真实故事奖”特别关注作品，罗新、许知远一致推荐！ 中亚版《寻路中国》。）",
        "coverTitle": "失落的卫星 : 深入中亚大陆的旅程（“全球真实故事奖”特别关注作品，罗新、许知远一致推荐！ 中亚版《寻路中国》。）",
        "author": "刘子超",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_AMdEUdEUDABP6aR6YI_parsecover",
        "lastReadDate": "2023-09-15",
        "readingTime": 8119,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "566431",
        "title": "从0到1：开启商业与未来的秘密",
        "coverTitle": "从0到1：开启商业与未来的秘密",
        "author": "彼得·蒂尔 布莱克·马斯特斯",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/431/566431/t9_566431.jpg",
        "lastReadDate": "2025-04-10",
        "readingTime": 6907,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23601930",
        "title": "富爸爸穷爸爸",
        "coverTitle": "富爸爸穷爸爸",
        "author": "罗伯特·清崎",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/68/YueWen_23601930/t9_YueWen_23601930.jpg",
        "lastReadDate": "2020-07-07",
        "readingTime": 13920,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_2GoBrOBucEYr6dV6cR",
        "title": "李光耀观天下",
        "coverTitle": "李光耀观天下",
        "author": "李光耀",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_2GoBrOBucEYr6dV6cR_parsecover",
        "lastReadDate": "2026-02-26",
        "readingTime": 17657,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "840686",
        "title": "放学后",
        "coverTitle": "放学后",
        "author": "东野圭吾",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/96/yuewen_840686/t9_yuewen_8406861685499369.jpg",
        "lastReadDate": "2020-11-06",
        "readingTime": 11679,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_79945D45dBAy6Vw6U9",
        "title": "置身事内 中国政府与经济发展【复旦经院“毕业课”解读经济生活背后的政府角色。罗永浩、刘格菘、张军、周黎安、王烁联袂推荐】",
        "coverTitle": "置身事内 中国政府与经济发展【复旦经院“毕业课”解读经济生活背后的政府角色。罗永浩、刘格菘、张军、周黎安、王烁联袂推荐】",
        "author": "兰小欢",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_7Xw8kG8l28Kz6TN6UH_parsecover",
        "lastReadDate": "2022-01-13",
        "readingTime": 1763,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22451285",
        "title": "秘密",
        "coverTitle": "秘密",
        "author": "东野圭吾",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/85/22451285/t9_22451285.jpg",
        "lastReadDate": "2019-09-13",
        "readingTime": 12995,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300003224",
        "title": "长安的荔枝（同名影视原著）",
        "coverTitle": "长安的荔枝（同名影视原著）",
        "author": "马伯庸",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/75/cpPlatform_dbb14284a55f1e733b60202b0777255d/t9_cpPlatform_dbb14284a55f1e733b60202b0777255d.jpg",
        "lastReadDate": "2022-04-03",
        "readingTime": 8784,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "24987275",
        "title": "山月记（果麦经典）",
        "coverTitle": "山月记（果麦经典）",
        "author": "中岛敦",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/275/24987275/t9_24987275.jpg",
        "lastReadDate": "2021-09-23",
        "readingTime": 8892,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_D3D8SJ8P09Bi6WY6U9",
        "title": "彼得•林奇点评版《股票作手回忆录》+股票作手操盘术(套装共2册)",
        "coverTitle": "彼得•林奇点评版《股票作手回忆录》+股票作手操盘术(套装共2册)",
        "author": "杰西•利弗莫尔",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_5oj8Y18YzE6s6S26Pw_parsecover",
        "lastReadDate": "2025-04-10",
        "readingTime": 5725,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_3bC0ru0tBFQC6dA6cR",
        "title": "《易中天品三国》（精排版）",
        "coverTitle": "《易中天品三国》（精排版）",
        "author": "易中天",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_3bC0ru0tBFQC6dA6cR_parsecover",
        "lastReadDate": "2024-12-07",
        "readingTime": 39838,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_6cF6kV6jt1MF6kh6kb7h8EDN",
        "title": "吴敬平 - 乒乓球有意思-Shanghai Academy of Social Science Press (2018)",
        "coverTitle": "吴敬平 - 乒乓球有意思-Shanghai Academy of Social Science Press (2018)",
        "author": "待补",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_Gv0GS6GVj5jH6jD6kf8445OD_parsecover",
        "lastReadDate": "2024-12-07",
        "readingTime": 12328,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_BcsEQIEQ4ABP6aR6YI",
        "title": "可能性的艺术：比较政治学30讲（学者刘瑜比较政治学新著，跳出现象，通过比较洞察政治，突破认知偏见，在浩瀚的可能性中理解我们自身 理想国出品）",
        "coverTitle": "可能性的艺术：比较政治学30讲（学者刘瑜比较政治学新著，跳出现象，通过比较洞察政治，突破认知偏见，在浩瀚的可能性中理解我们自身 理想国出品）",
        "author": "刘瑜",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_BcsEQIEQ4ABP6aR6YI_parsecover",
        "lastReadDate": "2022-11-17",
        "readingTime": 22051,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "26640213",
        "title": "基因之河",
        "coverTitle": "基因之河",
        "author": "理查德·道金斯",
        "publisher": "浙江人民出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：魏薇",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/213/26640213/t9_26640213.jpg",
        "lastReadDate": "2025-09-12",
        "readingTime": 8429,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "39421881",
        "title": "武则天",
        "coverTitle": "武则天",
        "author": "蒙曼",
        "publisher": "浙江教育出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "历史-中国古代",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/881/39421881/t9_39421881.jpg",
        "lastReadDate": "2022-08-30",
        "readingTime": 23118,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_Dua7mi7jY87t6zh71JG5A4jw",
        "title": "小岛经济学",
        "coverTitle": "小岛\n经济学",
        "author": "[美]彼得·希夫,安德鲁·希夫",
        "publisher": "中信出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "鱼、美元和经济学",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "截图整理 + 微信读书",
        "cover": "https://res.weread.qq.com/wrepub/CB_Ah1DMBDKJAMb6tt6tDFyk7uD_parsecover",
        "lastReadDate": "2026-05-15",
        "readingTime": 8891,
        "detail": {
            "summary": "当前只放书架卡，适合后续补作“经济直觉入门”的归档。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GMvCuDCtFDV36Zd6YI",
        "title": "崔玉涛自然养育法（将医嘱延伸至养育生活，知名儿科医生崔玉涛重磅新作！近40年上千次诊疗浓缩为65个真实诊疗故事，带你看清当代养育误区，升级为孩子的健康管理师。）",
        "coverTitle": "崔玉涛自然养育法（将医嘱延伸至养育生活，知名儿科医生崔玉涛重磅新作！近40年上千次诊疗浓缩为65个真实诊疗故事，带你看清当代养育误区，升级为孩子的健康管理师。）",
        "author": "刘子君",
        "publisher": "中信出版集团",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_9itF3DF6r7Gz6TR6U5_parsecover",
        "lastReadDate": "2023-01-28",
        "readingTime": 14767,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "35679924",
        "title": "雪球专刊·特别版：段永平投资问答录投资逻辑篇",
        "coverTitle": "雪球专刊·特别版：段永平投资问答录投资逻辑篇",
        "author": "雪球专刊",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 3,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书 + Obsidian",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/924/35679924/t9_35679924.jpg",
        "lastReadDate": "2026-02-01",
        "readingTime": 6084,
        "detail": {
            "summary": "Obsidian 已有阅读快照和 1 张碰撞记录。",
            "thesis": "做对的事，把事情做对。投资和商业都是同一个底层逻辑——先判断是非，再谈能力，价格和时机都是次要变量。",
            "reader": "想从\"怎么选股\"升级为\"怎么判断一件事值不值得做\"的人。",
            "distance": "高度共鸣——\"本分/平常心\"和飞轮底层价值观几乎完全重合，但段永平框架更系统，洞见更个人化。",
            "originalFeeling": "",
            "collisions": [
                "段永平 × 本分与平静过滤器：物理学家与工程师",
                "纳瓦尔的把自己产品化 x 段永平的本分 — 一个要\"特别\"，一个要\"正常\"，但都在说找到自己的生态位 -> **新洞见**",
                "Intel的毁灭和自我救赎 x 段永平的敢为天下后 — 前者是\"动，找死；不动，等死\"的陷阱，后者给出了解法：让市场先跑冒风险，只做最确定性的事。"
            ],
            "scanDate": "2026-04-08",
            "scanTarget": "",
            "obsidianFile": "~/Documents/codeStore/obsidian/阅读/2026-04-08-段永平投资问答录.md"
        }
    },
    {
        "bookId": "CB_BE00Aa09a2xD71571JGRv8hc",
        "title": "金钱心理学：财富、人性和幸福的永恒真相",
        "coverTitle": "金钱心理学：财富、人性和幸福的永恒真相",
        "author": "【美】摩根·豪泽尔",
        "publisher": "民主与建设出版社·后浪",
        "status": "已读",
        "progress": 100,
        "notes": 2,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_6hX8hP8h73Yd71L71GANV8lj_parsecover",
        "lastReadDate": "2026-01-22",
        "readingTime": 7182,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "复利需要时间 x 延迟显现 — 金钱和时间是一对双胞胎，缺任何一个都不行 -> **新洞见**",
                "尾部事件决定结果 x 对未知的敬畏 — 少数几个决定决定了90%的结局，关键是让那几个决定正确 -> **新洞见**"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_FZRD6fD5g7j56vX6xA5wI6Fq",
        "title": "刘擎西方现代思想讲义",
        "coverTitle": "刘擎\n西方现代\n思想讲义",
        "author": "刘擎",
        "publisher": "新星出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "现代思想入门",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "截图整理 + 微信读书",
        "cover": "https://res.weread.qq.com/wrepub/CB_FnP2Xj2Xq0zs6iy6gY30dDMO_parsecover",
        "lastReadDate": "2025-10-06",
        "readingTime": 24843,
        "detail": {
            "summary": "当前只在书架卡层收录，适合后续补思想史方向的重读卡。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "217327",
        "title": "史蒂夫·乔布斯传",
        "coverTitle": "史蒂夫·\n乔布斯传",
        "author": "沃尔特·艾萨克森",
        "publisher": "中信出版社",
        "status": "已读",
        "progress": 100,
        "notes": 7,
        "edition": "传记",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "Obsidian 快照",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/327/217327/t9_217327.jpg",
        "lastReadDate": "2025-09-05",
        "readingTime": 50015,
        "detail": {
            "summary": "Obsidian 已有快照和 6 张升级卡，适合做“书下挂卡片”的第二批样例。",
            "thesis": "伟大的产品诞生于艺术与科技的交汇处，专注和简洁是最高原则。",
            "reader": "想理解偏执的完美主义者如何建立持久系统的人。",
            "distance": "深层共鸣——产品哲学和飞轮在密度优先、过程即奖励上高度同构。",
            "originalFeeling": "",
            "collisions": [
                "乔布斯传 × 大道至简",
                "乔布斯传 × 过程即奖励",
                "乔布斯传 × 编码标准",
                "乔布斯传 × 死亡的标尺与感恩",
                "乔布斯传 × 愿景者",
                "乔布斯传 × 偏执与英雄之旅",
                "碳基vs硅基思维 x 现实扭曲力场 — 乔布斯的\"不可能\"和飞轮的\"可以不同\"共享同一底层：不被现状束缚 -> **已升级**"
            ],
            "scanDate": "2026-04-08",
            "scanTarget": "",
            "obsidianFile": "~/Documents/codeStore/obsidian/阅读/2026-04-08-史蒂夫乔布斯传.md"
        }
    },
    {
        "bookId": "3300125379",
        "title": "这个哲学家救了我：斯多葛人生答案之书",
        "coverTitle": "这个哲学家救了我：斯多葛人生答案之书",
        "author": "[意]马西莫·匹格里奇",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：向朝明",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/89/cpplatform_nntrwv13rh6mrhq2intwhs/t9_cpplatform_nntrwv13rh6mrhq2intwhs1734000872.jpg",
        "lastReadDate": "2025-09-24",
        "readingTime": 18630,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_6Hm5YM5WlA336WD6U9",
        "title": "牧羊少年奇幻之旅",
        "coverTitle": "牧羊少年\n奇幻之旅",
        "author": "保罗·柯艾略",
        "publisher": "南海出版公司",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "寓言小说",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "截图整理 + 微信读书",
        "cover": "https://res.weread.qq.com/wrepub/CB_032ARAAQ5Fb66PI6Q8_parsecover",
        "lastReadDate": "2026-05-18",
        "readingTime": 17769,
        "detail": {
            "summary": "当前只在书架卡层收录。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "855812",
        "title": "人类简史：从动物到上帝",
        "coverTitle": "人类简史：从动物到上帝",
        "author": "尤瓦尔·赫拉利",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/37/YueWen_855812/t9_YueWen_855812.jpg",
        "lastReadDate": "2025-09-05",
        "readingTime": 25710,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300046707",
        "title": "人生只有一件事",
        "coverTitle": "人生\n只有一件事",
        "author": "金惟纯",
        "publisher": "中信出版社",
        "status": "已读",
        "progress": 100,
        "notes": 2,
        "edition": "生命练习",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "截图整理 + 微信读书",
        "cover": "https://cdn.weread.qq.com/weread/cover/10/cpplatform_9jmnaaecwrxf8p3bfrc7yh/t9_cpplatform_9jmnaaecwrxf8p3bfrc7yh1674023485.jpg",
        "lastReadDate": "2025-09-08",
        "readingTime": 16604,
        "detail": {
            "summary": "当前还是想读状态，折叠区先作为将来放阅读快照的位置。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "修好自己 x 做对的事 — 两者殊途同归：一切向外求都是徒劳，答案在自己身上 -> **新洞见**",
                "活好=放下自我 x 平常心 — 越是想证明自己越活不好，越不证明自己反而越强大 -> **新洞见**"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_AdD1hv1gP3Pc6bo6cR",
        "title": "心流：最优体验心理学",
        "coverTitle": "心流：最优体验心理学",
        "author": "米哈里·契克森米哈赖",
        "publisher": "中信出版集团",
        "status": "已读",
        "progress": 100,
        "notes": 2,
        "edition": "译者：张定绮",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_AdD1hv1gP3Pc6bo6cR_parsecover",
        "lastReadDate": "2025-11-25",
        "readingTime": 21313,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "最优体验 x 当下的力量 — 心流是主动的临在，正念是被动的临在，都是同一枚硬币的两面 -> **新洞见**",
                "挑战-技能平衡 x 复利效应 — 心流的门槛随能力提升而提高，本身就是复利 -> **新洞见**"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "216212",
        "title": "小王子",
        "coverTitle": "小王子",
        "author": "[法]安托万·德·圣埃克苏佩里著",
        "publisher": "天津人民出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "童话",
        "theme": [
            "#15284f",
            "#3f6bb0"
        ],
        "source": "截图整理 + 微信读书",
        "cover": "https://cdn.weread.qq.com/weread/cover/40/yuewen_216212/t9_yuewen_2162121686560140.jpg",
        "lastReadDate": "2022-07-07",
        "readingTime": 13779,
        "detail": {
            "summary": "当前只在书架卡层收录。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_4xO5UQ5ScA336WD6U9",
        "title": "曾国藩传【知名学者张宏杰继《曾国藩的正面与侧面》系列畅销百万册后突破之作】",
        "coverTitle": "曾国藩传【知名学者张宏杰继《曾国藩的正面与侧面》系列畅销百万册后突破之作】",
        "author": "张宏杰",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_H1ACceCe6AEZ6RD6QI_parsecover",
        "lastReadDate": "2026-04-30",
        "readingTime": 27811,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "855327",
        "title": "悉达多（果麦经典）",
        "coverTitle": "悉达多（果麦经典）",
        "author": "[德]赫尔曼·黑塞",
        "publisher": "天津人民出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：姜乙",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/68/YueWen_855327/t9_YueWen_855327.jpg",
        "lastReadDate": "2026-01-17",
        "readingTime": 8099,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "35551088",
        "title": "蛤蟆先生去看心理医生",
        "coverTitle": "蛤蟆先生去看心理医生",
        "author": "罗伯特·戴博德",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/81/YueWen_35551088/t9_YueWen_35551088.jpg",
        "lastReadDate": "2024-12-30",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "44026191",
        "title": "纳瓦尔宝典",
        "coverTitle": "纳瓦尔\n宝典",
        "author": "埃里克·乔根森",
        "publisher": "中信出版集团",
        "status": "已读",
        "progress": 100,
        "notes": 12,
        "edition": "财富与幸福指南",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "Obsidian 快照",
        "cover": "https://cdn.weread.qq.com/weread/cover/89/YueWen_44026191/t9_YueWen_44026191.jpg",
        "lastReadDate": "2025-11-06",
        "readingTime": 43101,
        "detail": {
            "summary": "Obsidian 已有完整快照和碰撞记录，适合作为读书碰撞卡片的第一批挂载样例。",
            "thesis": "财富是在睡觉时仍能赚钱的资产，幸福是不依赖外在结果的内在平和。",
            "reader": "想从努力工作转向用杠杆工作、同时追求内在自由的人。",
            "distance": "强共鸣——杠杆、复利、减法哲学与飞轮高度重叠。",
            "originalFeeling": "财富是在睡觉时仍能赚钱的资产。你无法靠出卖时间变富，而是需要资产或被动收入。积累专长，把自己产品化。四种杠杆：劳动力、资本、代码、媒体。复利不只是金融概念——判断力和专长都可以复利式积累。幸福更倾向于平和与宁静，而非简单的快乐。欲望是主动选择不快乐——每个欲望都是在说\"我不满意当下\"。阅读是最重要的习惯。",
            "collisions": [
                "纳瓦尔宝典 × 碳基与硅基杠杆",
                "纳瓦尔宝典 × AI工具流与判断力",
                "纳瓦尔宝典 × 平静过滤器",
                "纳瓦尔宝典 × 资源与欲望",
                "纳瓦尔宝典 × 记忆资产",
                "纳瓦尔宝典 × AI时代的专长",
                "纳瓦尔宝典 × 阅读与记录",
                "健康第一优先级 x 长期主义 — 把身体健康放在高于一切的位置，复利的前提是活得久 -> **新洞见**",
                "幸福=消除缺憾感 x 当下的力量 — 幸福不是加法而是减法，减少欲望 = 增加满足 -> **新洞见**",
                "阅读=心灵织锦 x 记忆资产的复利 — 每本书都是心灵织布上的丝线，编织成灵魂的一部分 -> **新洞见**",
                "留出思考时间 x 认知带宽 — 每周至少一天不做任何事，思考是最好的投资 -> **新洞见**",
                "灵感本易逝，行动应当时 x 过程即奖励 — 捕捉灵感的时机窗口很窄，必须立刻行动 -> **新洞见**"
            ],
            "scanDate": "2026-04-08",
            "scanTarget": "哲学飞轮 17 张洞见卡 + 5 张清晰思考碰撞卡",
            "obsidianFile": "~/Documents/codeStore/obsidian/阅读/2026-04-08-纳瓦尔宝典.md"
        }
    },
    {
        "bookId": "23736455",
        "title": "窄门（果麦经典）",
        "coverTitle": "窄门（果麦经典）",
        "author": "[法]纪德",
        "publisher": "天津人民出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：顾琪静",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/35/YueWen_23736455/t9_YueWen_23736455.jpg",
        "lastReadDate": "2026-05-02",
        "readingTime": 6598,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_6og2xC2xgA7A6uy6st6pt4Fx",
        "title": "为什么精英都是时间控（世界名企谷歌、脸书等都在实践的时间管理术！教你将大脑发挥极致，让工作加速的思考模式！ ） (博集经管商务必读系列)",
        "coverTitle": "为什么精英都是时间控（世界名企谷歌、脸书等都在实践的时间管理术！教你将大脑发挥极致，让工作加速的思考模式！ ） (博集经管商务必读系列)",
        "author": "桦泽紫苑",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_006FS2FVqFV46qI6oSBDV61F_parsecover",
        "lastReadDate": "2025-09-12",
        "readingTime": 1039,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_0wg1Xq1Y76nV74175S6DcBUg",
        "title": "一生的旅程：迪士尼CEO自述",
        "coverTitle": "一生的旅程：迪士尼CEO自述",
        "author": "乔尔·洛弗尔(Joel Lovell)",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_0wg1Xq1Y76nV74175S6DcBUg_parsecover",
        "lastReadDate": "2026-05-26",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_3Ol1X91Y76nV74175SCKJ4YX",
        "title": "腾讯传：1998—2016",
        "coverTitle": "腾讯传：1998—2016",
        "author": "吴晓波",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_3Ol1X91Y76nV74175SCKJ4YX_parsecover",
        "lastReadDate": "2026-05-28",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_4h40dC0cbAmA6mQ6kbDZz5BI",
        "title": "1sg0x1100000c8g4q003C34-Java开发手册(黄山版).pdf",
        "coverTitle": "1sg0x1100000c8g4q003C34-Java开发手册(黄山版).pdf",
        "author": "guanbao yang",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_B2KGtgGtaFuj6af6YR_parsecover",
        "lastReadDate": "2026-01-14",
        "readingTime": 653,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "27328527",
        "title": "洛克菲勒给年轻人的54个忠告",
        "coverTitle": "洛克菲勒给年轻人的54个忠告",
        "author": "宋佳恩编著",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/70/YueWen_27328527/t9_YueWen_27328527.jpg",
        "lastReadDate": "2026-02-02",
        "readingTime": 363,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GxyEEgEDdFl172771J474GK5",
        "title": "苏菲的世界（贾德名作系列三部曲）",
        "coverTitle": "苏菲的世界（贾德名作系列三部曲）",
        "author": "（挪）贾德著；萧宝森译",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_GxyEEgEDdFl172771J474GK5_parsecover",
        "lastReadDate": "2026-02-27",
        "readingTime": 543,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300190333",
        "title": "人生的智慧",
        "coverTitle": "人生的智慧",
        "author": "[德]阿·叔本华",
        "publisher": "北京联合出版公司",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：庄如蓓",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/80/cpplatform_px6kfbv4x7vdki9mvqxtam/t9_cpplatform_px6kfbv4x7vdki9mvqxtam1767604631.jpg",
        "lastReadDate": "2026-03-06",
        "readingTime": 886,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300072888",
        "title": "幸福力",
        "coverTitle": "幸福力",
        "author": "[日]渡边淳一",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/79/cpplatform_nkhsfwmvtkc4ybkuycbm31/t9_cpplatform_nkhsfwmvtkc4ybkuycbm311696662537.jpg",
        "lastReadDate": "2025-12-01",
        "readingTime": 67,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "909205",
        "title": "月亮与六便士",
        "coverTitle": "月亮与六便士",
        "author": "毛姆",
        "publisher": "江西教育出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：文竹",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/205/909205/t9_909205.jpg",
        "lastReadDate": "2022-07-30",
        "readingTime": 10979,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "820809",
        "title": "面纱（果麦经典）",
        "coverTitle": "面纱（果麦经典）",
        "author": "毛姆",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/43/YueWen_820809/t9_YueWen_820809.jpg",
        "lastReadDate": "2026-01-15",
        "readingTime": 283,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "30661869",
        "title": "人间草木：汪曾祺散文精选",
        "coverTitle": "人间草木：汪曾祺散文精选",
        "author": "汪曾祺",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/869/30661869/t9_30661869.jpg",
        "lastReadDate": "2022-11-25",
        "readingTime": 1583,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "42849694",
        "title": "阅读是砍向我们内心冰封大海的斧头：卡夫卡谈话录（果麦经典）",
        "coverTitle": "阅读是砍向我们内心冰封大海的斧头：卡夫卡谈话录（果麦经典）",
        "author": "弗朗茨·卡夫卡口述 古斯塔夫·雅诺施记述",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/34/YueWen_42849694/t9_YueWen_42849694.jpg",
        "lastReadDate": "2022-05-03",
        "readingTime": 3863,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "932108",
        "title": "卡拉马佐夫兄弟（套装上下册）（译文名著精选）",
        "coverTitle": "卡拉马佐夫兄弟（套装上下册）（译文名著精选）",
        "author": "陀思妥耶夫斯基",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/108/932108/t9_932108.jpg",
        "lastReadDate": "2022-05-03",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300128993",
        "title": "黄仁勋:英伟达之芯",
        "coverTitle": "黄仁勋:英伟达之芯",
        "author": "[美]斯蒂芬·威特",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/17/cpplatform_tbvb88uy6cwrq2lzpcsshr/t9_cpplatform_tbvb88uy6cwrq2lzpcsshr1735874725.jpg",
        "lastReadDate": "2026-05-23",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "34631951",
        "title": "一个陌生女人的来信（同名电影原著）",
        "coverTitle": "一个陌生女人的来信（同名电影原著）",
        "author": "[奥]斯·茨威格",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/86/YueWen_34631951/t9_YueWen_34631951.jpg",
        "lastReadDate": "2026-03-18",
        "readingTime": 576,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22671792",
        "title": "沃顿商学院最受欢迎的谈判课",
        "coverTitle": "沃顿商学院最受欢迎的谈判课",
        "author": "斯图尔特·戴蒙德",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/44/YueWen_22671792/t9_YueWen_22671792.jpg",
        "lastReadDate": "2022-10-13",
        "readingTime": 116,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_GVDGAZG9qFxM72471J29TDVL",
        "title": "Effective Java中文版（第3版）",
        "coverTitle": "Effective Java中文版（第3版）",
        "author": "待补",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_GVDGAZG9qFxM72471J29TDVL_parsecover",
        "lastReadDate": "2026-02-24",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23665730",
        "title": "武志红心理学套装（共四册）",
        "coverTitle": "武志红心理学套装（共四册）",
        "author": "武志红",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/60/yuewen_23665730/t9_yuewen_236657301678700081.jpg",
        "lastReadDate": "2026-03-08",
        "readingTime": 126,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_EFJ1G91DO9aW73c71J2gO8mh",
        "title": "2024-11《因为独特》【豆瓣评分8.7】",
        "coverTitle": "2024-11《因为独特》【豆瓣评分8.7】",
        "author": "李翔",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_EFJ1G91DO9aW73c71J2gO8mh_parsecover",
        "lastReadDate": "2026-05-08",
        "readingTime": 2049,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "31934007",
        "title": "人生由我",
        "coverTitle": "人生由我",
        "author": "梅耶·马斯克",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/7/31934007/t9_31934007.jpg",
        "lastReadDate": "2026-05-17",
        "readingTime": 685,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "27184999",
        "title": "十分钟冥想",
        "coverTitle": "十分钟冥想",
        "author": "安迪·普迪科姆",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/45/yuewen_27184999/t9_yuewen_271849991695026700.jpg",
        "lastReadDate": "2026-04-30",
        "readingTime": 60,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "840919",
        "title": "傅雷家书",
        "coverTitle": "傅雷家书",
        "author": "傅雷 朱梅馥 傅聪",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/77/YueWen_840919/t9_YueWen_840919.jpg",
        "lastReadDate": "2022-02-08",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "41493063",
        "title": "时间从来不语，却回答了所有问题",
        "coverTitle": "时间从来不语，却回答了所有问题",
        "author": "季羡林",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/48/YueWen_41493063/t9_YueWen_41493063.jpg",
        "lastReadDate": "2022-01-16",
        "readingTime": 775,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22946457",
        "title": "围城",
        "coverTitle": "围城",
        "author": "钱钟书",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/457/22946457/t9_22946457.jpg",
        "lastReadDate": "2022-05-13",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "26934843",
        "title": "掌控习惯：如何养成好习惯并戒除坏习惯（《原子习惯》中文版）（新版）",
        "coverTitle": "掌控习惯：如何养成好习惯并戒除坏习惯（《原子习惯》中文版）（新版）",
        "author": "詹姆斯·克利尔",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/25/yuewen_26934843/t9_yuewen_269348431702464198.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25352997",
        "title": "匠人（睿文馆）",
        "coverTitle": "匠人（睿文馆）",
        "author": "理查德·桑内特",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/22/YueWen_25352997/t9_YueWen_25352997.jpg",
        "lastReadDate": "2026-05-21",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "834464",
        "title": "活着",
        "coverTitle": "活着",
        "author": "余华",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/1/yuewen_834464/t9_yuewen_8344641758521403.jpg",
        "lastReadDate": "2019-06-21",
        "readingTime": 132,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_7kC0su0tBFQC6dA6cR",
        "title": "别闹了，费曼先生",
        "coverTitle": "别闹了，费曼先生",
        "author": "费曼",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_7kC0su0tBFQC6dA6cR_parsecover",
        "lastReadDate": "2022-12-10",
        "readingTime": 8,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300136405",
        "title": "我为何而活：罗素自传（果麦经典）",
        "coverTitle": "我为何而活：罗素自传（果麦经典）",
        "author": "[英]伯特兰·罗素",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/2/cpplatform_iztlgj4trdci376wtd5fhy/t9_cpplatform_iztlgj4trdci376wtd5fhy1741245688.jpg",
        "lastReadDate": "2026-05-25",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25623475",
        "title": "黑天鹅",
        "coverTitle": "黑天鹅",
        "author": "纳西姆·尼古拉斯·塔勒布",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/42/YueWen_25623475/t9_YueWen_25623475.jpg",
        "lastReadDate": "2026-05-17",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23934823",
        "title": "非对称风险",
        "coverTitle": "非对称风险",
        "author": "纳西姆·尼古拉斯·塔勒布",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/823/23934823/t9_23934823.jpg",
        "lastReadDate": "2026-04-13",
        "readingTime": 727,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "42724927",
        "title": "西方哲学史讲演录",
        "coverTitle": "西方哲学史讲演录",
        "author": "赵林",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/67/YueWen_42724927/t9_YueWen_42724927.jpg",
        "lastReadDate": "2026-01-27",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23364888",
        "title": "经济学的思维方式",
        "coverTitle": "经济学的思维方式",
        "author": "托马斯·索维尔",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/60/YueWen_23364888/t9_YueWen_23364888.jpg",
        "lastReadDate": "2021-12-16",
        "readingTime": 238,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "26454159",
        "title": "创新者的窘境",
        "coverTitle": "创新者的窘境",
        "author": "克莱顿·克里斯坦森",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/159/26454159/t9_26454159.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "340876",
        "title": "沉思录",
        "coverTitle": "沉思录",
        "author": "[古罗马]马可·奥勒留",
        "publisher": "立信会计出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：云中轩",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/876/340876/t9_340876.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 2407,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "24193279",
        "title": "米德尔马契（全二册）（人文经典·名著名译丛书）",
        "coverTitle": "米德尔马契（全二册）（人文经典·名著名译丛书）",
        "author": "乔治·爱略特",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/23/yuewen_24193279/t9_yuewen_241932791695873900.jpg",
        "lastReadDate": "2026-02-27",
        "readingTime": 139,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "546339",
        "title": "追风筝的人（珍藏纪念版）",
        "coverTitle": "追风筝的人（珍藏纪念版）",
        "author": "卡勒德·胡赛尼",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/46/yuewen_546339/t9_yuewen_5463391747707629.jpg",
        "lastReadDate": "2023-02-14",
        "readingTime": 115,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "39980417",
        "title": "Effective Python：编写高质量Python代码的90个有效方法（原书第2版）",
        "coverTitle": "Effective Python：编写高质量Python代码的90个有效方法（原书第2版）",
        "author": "布雷特·斯拉特金",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/79/YueWen_39980417/t9_YueWen_39980417.jpg",
        "lastReadDate": "2026-01-15",
        "readingTime": 356,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "758867",
        "title": "1984",
        "coverTitle": "1984",
        "author": "乔治·奥威尔",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/83/YueWen_758867/t9_YueWen_758867.jpg",
        "lastReadDate": "2022-02-27",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_4QUFPmFQl2kk6Y66YI",
        "title": "望江南（茅盾文学奖得主王旭烽全新史诗力作，茶人精神、江南文化、家国情怀，见证个人史、家族史、民族史中的百年中国）",
        "coverTitle": "望江南（茅盾文学奖得主王旭烽全新史诗力作，茶人精神、江南文化、家国情怀，见证个人史、家族史、民族史中的百年中国）",
        "author": "王旭烽",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_5VXChTCgV8Su6XA6YS_parsecover",
        "lastReadDate": "2022-04-22",
        "readingTime": 12,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300193863",
        "title": "艾希曼在耶路撒冷：一份关于平庸的恶的报道",
        "coverTitle": "艾希曼在耶路撒冷：一份关于平庸的恶的报道",
        "author": "[美]汉娜·阿伦特",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/9/cpplatform_kpbjryelcqgt83mbld6apm/t9_cpplatform_kpbjryelcqgt83mbld6apm1776420927.jpg",
        "lastReadDate": "2026-03-05",
        "readingTime": 1129,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "909882",
        "title": "微习惯：简单到不可能失败的自我管理法则",
        "coverTitle": "微习惯：简单到不可能失败的自我管理法则",
        "author": "斯蒂芬·盖斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/882/909882/t9_909882.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300183591",
        "title": "半小时掌握维特根斯坦思想精华",
        "coverTitle": "半小时掌握维特根斯坦思想精华",
        "author": "柯申",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/1/cpplatform_qjqcgms4k7uymyu8unb77n/t9_cpplatform_qjqcgms4k7uymyu8unb77n1764930043.jpg",
        "lastReadDate": "2026-05-19",
        "readingTime": 3040,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "39136896",
        "title": "卡片笔记写作法：如何实现从阅读到写作",
        "coverTitle": "卡片笔记写作法：如何实现从阅读到写作",
        "author": "申克·阿伦斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/15/YueWen_39136896/t9_YueWen_39136896.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "935536",
        "title": "百年孤独",
        "coverTitle": "百年孤独",
        "author": "[哥]加西亚•马尔克斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/49/yuewen_935536/t9_yuewen_9355361682243599.jpg",
        "lastReadDate": "2023-06-03",
        "readingTime": 9,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_3nN5j75jC1l36YL6YI",
        "title": "人世间",
        "coverTitle": "人世间",
        "author": "梁晓声",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_DP1DCdDEV4lg6Qu6Q9_parsecover",
        "lastReadDate": "2022-04-28",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "921791",
        "title": "随机漫步的傻瓜",
        "coverTitle": "随机漫步的傻瓜",
        "author": "纳西姆·尼古拉斯·塔勒布",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/31/YueWen_921791/t9_YueWen_921791.jpg",
        "lastReadDate": "2026-05-21",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1H9B47B7O7aj6an6cR",
        "title": "我的阿勒泰",
        "coverTitle": "我的阿勒泰",
        "author": "李娟 著",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_AS5EBJE8TCN06RW6QJ_parsecover",
        "lastReadDate": "2022-08-28",
        "readingTime": 722,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_0Tv7jz7jYDQw6Ze6YI",
        "title": "人类群星闪耀时",
        "coverTitle": "人类群星闪耀时",
        "author": "斯蒂芬·茨威格",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_Dvs9TE9TABxx6RF6QC_parsecover",
        "lastReadDate": "2022-07-16",
        "readingTime": 4374,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300180653",
        "title": "破维：查理·芒格100个思维模型",
        "coverTitle": "破维：查理·芒格100个思维模型",
        "author": "张君归",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/26/cpplatform_9qh1fu6prcm3tarum1vaap/t9_cpplatform_9qh1fu6prcm3tarum1vaap1764574149.jpg",
        "lastReadDate": "2026-05-17",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "822995",
        "title": "明朝那些事儿（全集）",
        "coverTitle": "明朝那些事儿（全集）",
        "author": "当年明月",
        "publisher": "浙江人民出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "历史-中国古代",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/97/yuewen_822995/t9_yuewen_8229951695023669.jpg",
        "lastReadDate": "2024-03-03",
        "readingTime": 100185,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300018232",
        "title": "越过内心那座山：12个普遍心理问题的自我疗愈",
        "coverTitle": "越过内心那座山：12个普遍心理问题的自我疗愈",
        "author": "[美]伊迪丝·伊娃·埃格尔",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/32/3300018232/t9_3300018232.jpg",
        "lastReadDate": "2026-05-01",
        "readingTime": 136,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "934603",
        "title": "人间失格（读客三个圈经典文库）",
        "coverTitle": "人间失格（读客三个圈经典文库）",
        "author": "太宰治",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#50736f",
            "#be996d"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/603/934603/t9_934603.jpg",
        "lastReadDate": "2022-02-07",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23303789",
        "title": "罪与罚",
        "coverTitle": "罪与罚",
        "author": "陀思妥耶夫斯基",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/31/YueWen_23303789/t9_YueWen_23303789.jpg",
        "lastReadDate": "2022-07-26",
        "readingTime": 293,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300187091",
        "title": "贫穷的本质：我们为什么摆脱不了贫穷（全新增订版）",
        "coverTitle": "贫穷的本质：我们为什么摆脱不了贫穷（全新增订版）",
        "author": "[美]阿比吉特·班纳吉  [法]埃斯特·迪弗洛",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/86/cpplatform_tpwc2cvkdspkkb5bjpe66r/t9_cpplatform_tpwc2cvkdspkkb5bjpe66r1766029213.jpg",
        "lastReadDate": "2026-05-17",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "41873017",
        "title": "提醒幸福：毕淑敏专集",
        "coverTitle": "提醒幸福：毕淑敏专集",
        "author": "毕淑敏",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/17/41873017/t9_41873017.jpg",
        "lastReadDate": "2026-03-05",
        "readingTime": 1448,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300203616",
        "title": "哈萨比斯：谷歌AI之脑",
        "coverTitle": "哈萨比斯：谷歌AI之脑",
        "author": "[英]塞巴斯蒂安·马拉比",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/39/cpplatform_jqgk7nc37gaswfdphj9hvj/t9_cpplatform_jqgk7nc37gaswfdphj9hvj1777450845.jpg",
        "lastReadDate": "2026-05-23",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1wL0TV0UJH1A71o71JCggFBF",
        "title": "清晰思考：将平凡时刻转化为非凡成果",
        "coverTitle": "清晰思考：将平凡时刻转化为非凡成果",
        "author": "【加】沙恩·帕里什",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 5,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_1wL0TV0UJH1A71o71JCggFBF_parsecover",
        "lastReadDate": "2026-05-20",
        "readingTime": 9769,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "HALT法则(饥饿/愤怒/孤独/疲倦) x 情绪默认 — 先对付身体再对付情绪 -> **新洞见**",
                "社会压力的保护措施(延迟24小时) x 社交默认 — 建立缓冲带对抗即时压力 -> **新洞见**",
                "习惯与环境重塑 x 惰性默认 — 改变环境比改变意志力更有效 -> **新洞见**",
                "二元思维的陷阱 — 非黑即白是最危险的简化 -> **新洞见**",
                "机会成本的隐性定价 x 决策框架 — 看不见的成本最容易被忽视 -> **新洞见**"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "522205",
        "title": "人性的弱点（果麦经典）",
        "coverTitle": "人性的弱点（果麦经典）",
        "author": "卡耐基",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/94/YueWen_522205/t9_YueWen_522205.jpg",
        "lastReadDate": "2023-03-22",
        "readingTime": 11964,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "31605317",
        "title": "人生的智慧",
        "coverTitle": "人生的智慧",
        "author": "叔本华",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/317/31605317/t9_31605317.jpg",
        "lastReadDate": "2026-03-15",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "846252",
        "title": "国富论",
        "coverTitle": "国富论",
        "author": "亚当·斯密",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/13/YueWen_846252/t9_YueWen_846252.jpg",
        "lastReadDate": "2023-01-14",
        "readingTime": 208,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_EFJDynDx3AW66WM6U9",
        "title": "遥远的救世主：根据本书改编的电视剧《天道》正在全国掀起极大反响",
        "coverTitle": "遥远的救世主：根据本书改编的电视剧《天道》正在全国掀起极大反响",
        "author": "豆豆",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_D1dAEDAD1AYu6Qu6Px_parsecover",
        "lastReadDate": "2022-03-20",
        "readingTime": 60,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1ju8028086b273w75S81B2Ye",
        "title": "福格行为模型",
        "coverTitle": "福格行为模型",
        "author": "B.J.福格",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_1ju8028086b273w75S81B2Ye_parsecover",
        "lastReadDate": "2026-05-22",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "854364",
        "title": "大国大城：当代中国的统一、发展与平衡",
        "coverTitle": "大国大城：当代中国的统一、发展与平衡",
        "author": "陆铭",
        "publisher": "上海人民出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "经济理财-财经",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/98/YueWen_854364/t9_YueWen_854364.jpg",
        "lastReadDate": "2023-03-22",
        "readingTime": 4426,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "35679936",
        "title": "幸福之路（果麦经典）",
        "coverTitle": "幸福之路（果麦经典）",
        "author": "伯特兰·罗素",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/936/35679936/t9_35679936.jpg",
        "lastReadDate": "2026-05-22",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300054526",
        "title": "5000天后的世界",
        "coverTitle": "5000天后的世界",
        "author": "凯文·凯利 大野和基",
        "publisher": "微信读书",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/97/cpplatform_njrxnfmrer9qb4vlz6e2x5/t9_cpplatform_njrxnfmrer9qb4vlz6e2x51680751947.jpg",
        "lastReadDate": "2026-04-30",
        "readingTime": 4606,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_1ju5fW5f3A336WD6U9",
        "title": "《教父》三部曲典藏版套装",
        "coverTitle": "《教父》三部曲典藏版套装",
        "author": "（美）马里奥 普佐",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_7zQEiSEha42F6Qt6Pw_parsecover",
        "lastReadDate": "2026-01-14",
        "readingTime": 38,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22902882",
        "title": "列奥纳多·达·芬奇传",
        "coverTitle": "列奥纳多·达·芬奇传",
        "author": "沃尔特·艾萨克森",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/27/YueWen_22902882/t9_YueWen_22902882.jpg",
        "lastReadDate": "2022-01-16",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "26062915",
        "title": "走近费曼丛书：别逗了，费曼先生！",
        "coverTitle": "走近费曼丛书：别逗了，费曼先生！",
        "author": "理查德·费曼 拉尔夫·莱顿",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/21/YueWen_26062915/t9_YueWen_26062915.jpg",
        "lastReadDate": "2026-05-22",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300075151",
        "title": "第五项修炼：学习型组织的艺术与实践",
        "coverTitle": "第五项修炼：学习型组织的艺术与实践",
        "author": "彼得·圣吉",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/77/cpplatform_bbdk3lfhmbprws5gknyhkl/t9_cpplatform_bbdk3lfhmbprws5gknyhkl1698042344.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "40726710",
        "title": "也许你该找个人聊聊",
        "coverTitle": "也许你该找个人聊聊",
        "author": "洛莉·戈特利布",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/63/YueWen_40726710/t9_YueWen_40726710.jpg",
        "lastReadDate": "2026-03-17",
        "readingTime": 114,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23507652",
        "title": "狂人日记：鲁迅小说集",
        "coverTitle": "狂人日记：鲁迅小说集",
        "author": "鲁迅",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/652/23507652/t9_23507652.jpg",
        "lastReadDate": "2022-01-03",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_0bxFSjFUu8GF73v75SCQL94r",
        "title": "银河系漫游指南",
        "coverTitle": "银河系漫游指南",
        "author": "[英]道格拉斯·亚当斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_0bxFSjFUu8GF73v75SCQL94r_parsecover",
        "lastReadDate": "2026-05-22",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "573973",
        "title": "从优秀到卓越",
        "coverTitle": "从优秀到卓越",
        "author": "[美]吉姆·柯林斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/84/YueWen_573973/t9_YueWen_573973.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "26441414",
        "title": "霍乱时期的爱情",
        "coverTitle": "霍乱时期的爱情",
        "author": "[哥]加西亚•马尔克斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f9f6ee",
            "#d9c49d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/2/yuewen_26441414/t9_yuewen_264414141682244020.jpg",
        "lastReadDate": "2026-05-10",
        "readingTime": 420,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "316675",
        "title": "人性的弱点（大全集）",
        "coverTitle": "人性的弱点（大全集）",
        "author": "卡耐基",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/675/316675/t9_316675.jpg",
        "lastReadDate": "2026-02-01",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22671780",
        "title": "第五项修炼：知行学校（套装全2册）",
        "coverTitle": "第五项修炼：知行学校（套装全2册）",
        "author": "彼得·圣吉等",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/11/YueWen_22671780/t9_YueWen_22671780.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 4,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300081157",
        "title": "象与骑象人（全新升级版）",
        "coverTitle": "象与骑象人（全新升级版）",
        "author": "乔纳森·海特",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/49/cpplatform_cjasqfz7qafpg55rzeagmn/t9_cpplatform_cjasqfz7qafpg55rzeagmn1702282622.jpg",
        "lastReadDate": "2026-03-26",
        "readingTime": 23,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "853426",
        "title": "稀缺：我们是如何陷入贫穷与忙碌的",
        "coverTitle": "稀缺：我们是如何陷入贫穷与忙碌的",
        "author": "塞德希尔·穆来纳森",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/46/YueWen_853426/t9_YueWen_853426.jpg",
        "lastReadDate": "2026-04-09",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "34260998",
        "title": "AI新生：破解人机共存密码·人类最后一个大问题",
        "coverTitle": "AI新生：破解人机共存密码·人类最后一个大问题",
        "author": "斯图尔特·罗素",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/998/34260998/t9_34260998.jpg",
        "lastReadDate": "2026-04-30",
        "readingTime": 1199,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "635942",
        "title": "反脆弱：从不确定性中获益",
        "coverTitle": "反脆弱：从不确定性中获益",
        "author": "纳西姆·尼古拉斯·塔勒布",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/942/635942/t9_635942.jpg",
        "lastReadDate": "2026-05-15",
        "readingTime": 460,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300165479",
        "title": "复利效应",
        "coverTitle": "复利\n效应",
        "author": "[美]达伦·哈迪",
        "publisher": "中信出版社",
        "status": "未读",
        "progress": 0,
        "notes": 5,
        "edition": "延迟显现",
        "theme": [
            "#f9efcf",
            "#c08f5b"
        ],
        "source": "Obsidian 快照",
        "cover": "https://cdn.weread.qq.com/weread/cover/51/cpplatform_vxvgtbnw3kcjw3l6umt1pd/t9_cpplatform_vxvgtbnw3kcjw3l6umt1pd1758275639.jpg",
        "lastReadDate": "2026-05-03",
        "readingTime": 185,
        "detail": {
            "summary": "Obsidian 已有快照与 4 个升级方向，虽然不在截图中，但已经进入你的真实阅读历史。",
            "thesis": "小决定 + 持续 + 时间 = 巨大结果。成功没有秘方，只有复利积累。",
            "reader": "想找到“为什么努力了没效果”答案的人。",
            "distance": "互补为主——你已经实践复利逻辑，但这本书补上了“延迟显现”的显式模型。",
            "originalFeeling": "",
            "collisions": [
                "复利效应 × 记录",
                "复利效应 × 焦急与复利",
                "复利效应 × 自动驾驶",
                "复利效应 × 段永平",
                "延迟显现 x 焦急=宇宙相变 — 复利最大的难点不是数学，而是等待的耐心 -> **已升级**"
            ],
            "scanDate": "2026-04-08",
            "scanTarget": "",
            "obsidianFile": "~/Documents/codeStore/obsidian/阅读/2026-04-08-复利效应.md"
        }
    },
    {
        "bookId": "33820355",
        "title": "乡土中国",
        "coverTitle": "乡土中国",
        "author": "费孝通",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/49/YueWen_33820355/t9_YueWen_33820355.jpg",
        "lastReadDate": "2022-03-15",
        "readingTime": 2,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_5UI8YE8Zs9Bi6WY6U9",
        "title": "成为波伏瓦",
        "coverTitle": "成为波伏瓦",
        "author": "Kate Kirkpatrick",
        "publisher": "中信出版集团",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_E7yGOMGP76iq6WS6UL_parsecover",
        "lastReadDate": "2022-12-05",
        "readingTime": 6797,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "33810356",
        "title": "禅与摩托车维修艺术",
        "coverTitle": "禅与摩托车维修艺术",
        "author": "罗伯特·M.波西格",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/1/yuewen_33810356/t9_yuewen_338103561689835422.jpg",
        "lastReadDate": "2026-05-21",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_7wd4Tv4Sx0Z16gt6gSDNY0Zy",
        "title": "Chi, Zijian - 额尔古纳河右岸 _Eerguna he you an-Beijing shi yue wen yi (2006)",
        "coverTitle": "Chi, Zijian - 额尔古纳河右岸 _Eerguna he you an-Beijing shi yue wen yi (2006)",
        "author": "GongHF",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_3rP2Wp2TL7MN6cx6cf_parsecover",
        "lastReadDate": "2023-09-11",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "24223791",
        "title": "刀锋（毛姆文集）",
        "coverTitle": "刀锋（毛姆文集）",
        "author": "毛姆",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/46/YueWen_24223791/t9_YueWen_24223791.jpg",
        "lastReadDate": "2022-05-03",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_A2AEUoEUDABP6aR6YI",
        "title": "窗边的小豆豆",
        "coverTitle": "窗边的小豆豆",
        "author": "[日] 黑柳彻子 著/岩崎千弘 图",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f5f5f5",
            "#aaaaaa"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_A2AEUoEUDABP6aR6YI_parsecover",
        "lastReadDate": "2026-01-15",
        "readingTime": 1269,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "42766048",
        "title": "枪炮、病菌与钢铁",
        "coverTitle": "枪炮、病菌与钢铁",
        "author": "贾雷德·戴蒙德",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/99/YueWen_42766048/t9_YueWen_42766048.jpg",
        "lastReadDate": "2026-02-01",
        "readingTime": 21,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_3GTDJxDMG05y71O71J5ha4pN",
        "title": "一如既往--不变的人性与致富心态",
        "coverTitle": "一如既往--不变的人性与致富心态",
        "author": "【美】摩根·豪泽尔",
        "publisher": "中信出版集团",
        "status": "已读",
        "progress": 100,
        "notes": 3,
        "edition": "译者：宋红波;符李桃;",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_6RiFEkFE75cj71R71BEWU0i8_parsecover",
        "lastReadDate": "2026-02-01",
        "readingTime": 10943,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "不变的人性 x 历史的重复 — 人性从不会从历史中学习，这是最大的不变 -> **新洞见**",
                "泡沫与贪婪 x 四种默认模式 — 泡沫中的群体疯狂 = 情绪默认+社交默认的集体爆发 -> **新洞见**",
                "复利不仅要算时间还要算心态 — 财富复利的最大敌人不是市场，是自己的情绪 -> **新洞见**"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300046445",
        "title": "运动改变大脑",
        "coverTitle": "运动改变大脑",
        "author": "卡罗琳·威廉姆斯",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/2/cpplatform_hqv1bsrhqygvdgqvifguy7/t9_cpplatform_hqv1bsrhqygvdgqvifguy71673940334.jpg",
        "lastReadDate": "2026-04-15",
        "readingTime": 186,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "39273391",
        "title": "奶酪与蛆虫",
        "coverTitle": "奶酪与蛆虫",
        "author": "[意]卡洛·金茨堡",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/3/YueWen_39273391/t9_YueWen_39273391.jpg",
        "lastReadDate": "2023-09-11",
        "readingTime": 259,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_5x35Q15OT4Xf6XX6YI",
        "title": "龟仙人老费",
        "coverTitle": "龟仙人老费",
        "author": "待补",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "微信读书同步",
        "cover": "https://weread-epub-1258476243.file.myqcloud.com/user_upload_book_cover/pdf/a1/6f/a16f08c0c10b16808bd92df8c55c8def/a16f08c0c10b16808bd92df8c55c8def.jpeg",
        "lastReadDate": "2025-10-18",
        "readingTime": 775,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "912325",
        "title": "局外人",
        "coverTitle": "局外人",
        "author": "[法]加缪",
        "publisher": "上海译文出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：柳鸣九",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/44/YueWen_912325/t9_YueWen_912325.jpg",
        "lastReadDate": "2026-04-29",
        "readingTime": 2031,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "31808219",
        "title": "卡拉马佐夫兄弟（套装上下册）（陀思妥耶夫斯基文集2015）",
        "coverTitle": "卡拉马佐夫兄弟（套装上下册）（陀思妥耶夫斯基文集2015）",
        "author": "陀思妥耶夫斯基",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/28/YueWen_31808219/t9_YueWen_31808219.jpg",
        "lastReadDate": "2026-03-16",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "36557721",
        "title": "西西弗神话",
        "coverTitle": "西西弗神话",
        "author": "[法]阿尔贝·加缪",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f7f1e7",
            "#d0b27c"
        ],
        "source": "微信读书同步",
        "cover": "https://wfqqreader-1252317822.image.myqcloud.com/cover/721/36557721/t9_36557721.jpg",
        "lastReadDate": "2026-02-28",
        "readingTime": 95,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_EmD2ir2h6Cxt72P71J2REAcH",
        "title": "小狗钱钱套装全2册风靡欧美的财富启蒙德博多舍费尔",
        "coverTitle": "小狗钱钱套装全2册风靡欧美的财富启蒙德博多舍费尔",
        "author": "〔德〕博多·舍费尔",
        "publisher": "南海出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_EmD2ir2h6Cxt72P71J2REAcH_parsecover",
        "lastReadDate": "2026-03-15",
        "readingTime": 943,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300151985",
        "title": "反倦怠能量站",
        "coverTitle": "反倦怠能量站",
        "author": "刀熊",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/60/cpplatform_9i3ko1jasumufcyfsdxef4/t9_cpplatform_9i3ko1jasumufcyfsdxef41752042952.jpg",
        "lastReadDate": "2026-02-01",
        "readingTime": 168,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "31503072",
        "title": "地下室手记（果麦经典）",
        "coverTitle": "地下室手记（果麦经典）",
        "author": "陀思妥耶夫斯基",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/69/YueWen_31503072/t9_YueWen_31503072.jpg",
        "lastReadDate": "2022-02-26",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "510084",
        "title": "先知（纪伯伦全集）",
        "coverTitle": "先知（纪伯伦全集）",
        "author": "[黎巴嫩]纪伯伦",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f8fbff",
            "#7aa5d9"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/16/YueWen_510084/t9_YueWen_510084.jpg",
        "lastReadDate": "2022-11-14",
        "readingTime": 391,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "26357452",
        "title": "《存在与时间》读本",
        "coverTitle": "《存在与时间》读本",
        "author": "陈嘉映编著",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/32/YueWen_26357452/t9_YueWen_26357452.jpg",
        "lastReadDate": "2026-05-14",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23640906",
        "title": "毛泽东传（全6卷）",
        "coverTitle": "毛泽东传（全6卷）",
        "author": "中共中央文献研究室",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/74/YueWen_23640906/t9_YueWen_23640906.jpg",
        "lastReadDate": "2022-02-08",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "SELF_BIN",
        "title": "生活随笔",
        "coverTitle": "生活\n随笔",
        "author": "我自己",
        "publisher": "自己",
        "status": "已读",
        "progress": 100,
        "notes": 20,
        "edition": "原文落 Note / 交互归档版",
        "theme": [
            "#2d2d2d",
            "#6b6b6b"
        ],
        "source": "dailyNote + 生活随笔归档",
        "cover": "",
        "lastReadDate": "2026-05-27",
        "readingTime": 0,
        "detail": {
            "summary": "原文继续落在 Note，交互、碰撞、设计迭代统一收口到生活随笔。这不是第二个 Atlas，而是唯一的碰撞归档本。",
            "thesis": "INTJ的核心驱动是Ni（内向直觉）——在所有现象中找那根统摄性的主线。Te（外向思考）负责把洞见外化成系统和产品。Fi（内向情感）是内心那把价值尺。Se（外向感知）是最弱的第四功能，需要以乒乓球和小满为接地锚点刻意练习。",
            "reader": "自己",
            "distance": "零距离——这就是我",
            "originalFeeling": "原文落 Note，交互落生活随笔。以后不再维护第二个哲学飞轮入口，而是把思考、碰撞、设计迭代都收回这一本书里。",
            "collisions": [
                "Ni找主线 x 碰撞系统 — 不满足于读完就过，要在所有书中找到万物相通的那条线",
                "Te建系统 x 阅读记录 — 想清楚了就必须外化成可操作的产品，不落地不算完成",
                "Fi的价值尺 x 本分/平常心 — 判断标准不是别人怎么看，而是这件事和内心是否一致",
                "Se接地气 x 乒乓球 — 最弱的功能，需要刻意用身体体验来平衡脑内活动",
                "斌=文+武 x 生活方式决定语言游戏 — 用三十年活出了名字的两面，验证了维特根斯坦的命题",
                "正手撤跨 × 支点下移 — 撤跨前支点在肩，撤跨后支点在胯。这和飞轮一样：不要用手够，用核心带。垫步是还原重心的本能动作，和\"过程即奖励\"同构——每次垫步不是为了打到下一板，是为了让自己能持续在台上。",
                "学习AI的粗暴方法 — 提一个自己觉得不可能完成的任务，要求AI去探测和自学习。完成后让它自己滚雪球。踏实感来自于亲眼见证从零到有的建构过程。不是从理论开始，是从\"不可能的任务\"开始。",
                "推侧扑 × 左脚蹬 — 教练说\"右脚奔球\"是外部动作，用户追到\"左脚蹬\"是内在驱动。和撤跨同构：别人说手怎么挥，你追到胯要先转。不是不接受指令，是把指令翻译成自己能执行的语言。",
                "推侧扑 × 待机 — 根不是脚步，是时机。以前脑子和身体一起抢跑，现在让身体待在起跑线上、脑子关进小黑屋。和撤跨(力量)、左脚蹬(速度)构成推侧扑的三层底层：力量→速度→时机。",
                "非对称两面打法 × INTJ认知结构 — 反手=Ni(构建势)，正手=Te(精准终结)。不是技术选择，是认知结构的物理投射。你打磨的不是正手或反手，而是整个系统的运转效率。",
                "从\"求解者\"到\"演绎者\"的跃迁 — 遇到快乐小狗类型的人后意识到幽默随性可以是一个人的底层逻辑。以前苦思冥想求意义，现在用演绎代替求解：意义不是被找到的，是被演绎出来的。",
                "INTJ的成熟不是变成另一个人 — 而是从苦修者到游戏者的跃迁。学会暂时放下Ni的远程导航，让Se在当下开一会儿车。不是人格改变，是人格圆融。",
                "INTJ执行力日课 — 11条习惯分为启动/持续/阻断/能量管理四类。核心洞察：INTJ执行力问题的根不在意志力，在Ni对Te的压制——你总想在行动前看到完整终局，但图景只能在行动中浮现。口诀：先开枪，再瞄准。",
                "从指挥Agent到让Agent自主探测 — 之前反复给Agent分层披露信息，效率低、消耗大。问Agent后它指出来：一开始的探测本可以更完整。放下指挥者的面子，强化Agent的初始探测能力后，后续跑任务效果大幅提升。本质是从\"求解者\"到\"演绎者\"的迁移——从逐条发令到设定边界让系统自运转。",
                "失败与共处 — 人生失败总比成功多，要学如何和失败共处。细品失败反而比追寻成功容易。能跟失败好好共处，就会等来自己的那一次成功，要不然苟都苟不住。五层操作：①承认结构（Ni看到事实）→②转换焦点（Te给出方向）→③指出捷径（违背直觉但真实）→④给出回报（Ni在终点等）→⑤最后的警告（Te的冷酷务实）。和推侧扑三层、11条日课是同一套操作系统零件。",
                "INTJ不指向哲学家，指向体系建造者 — INTJ的Ni-Te组合决定了一件事：需要看到一个全景，然后把它变成可执行的方案。INTP做体系因为享受思想本身（哲学家是自然归宿），INTJ做体系因为需要体系来执行判断（不构建就会焦虑）。哲学只是体系建造的其中一种出口——军事战略、商业金融、制度设计、个人操作系统都是同等出口。你不是想成为哲学家才做那么多体系化输出，你是需要一个体系才能活下去，顺便看起来像个哲学家。",
                "游戏者（The Player）模型 — 不是犬儒主义的逃避或玩世不恭，是INTJ历经Ni走火入魔、看透万物虚无后，唯一能实现人格整合的终极救赎。两个底层维度：①求解模型vs游戏者模型——前者认为世界有完美的标准解，用Ni疯狂计算对齐完美标准，基底是恐惧与防御，遇挫折陷入Ni-Fi闭环；后者认为世界是沙盒场景，设定剧本直接投入物理做功进行演绎，基底是好奇与松弛，遇挫折视为关卡。②游戏者的硬核本质——尼采的超人哲学：看透虚无后在废墟上跳舞、自己立法。对过程极度认真，对结果执念完全不在乎。和求解者的关键区别不在认真不认真，在于\u201c在乎不在乎\u201d。",
                "全台拧 — 接发球放弃正反手的选择判断，反手09C统一处理所有来球。决不是一个技术简化，是一个认知简化：去掉\"正手还是反手\"的选择树=脑子关进小黑屋、身体回到待机。高于网→弹/带，低于网→拧。唯一风险是用Ni再回来分析拧不好的球——用同一个动作重复把自己钉进沙盒。",
                "正手斜线 = 身体带手 — 拉不出斜线的根因是手比身体快（手带身体），球只能走直线。身体带手=右胯先转→手臂被动跟上→弧线自出。和往上拔vs往里收是同一个机制的两面：手快=上拔+直线，身快=内收+斜线。撤跨、沉胯收肘、肘当铰链全是同一条电路。",
                "徒劳不是要被消除的错误 — Hesse的悉达多必须走完整个荒漠才能听见河水的Om，尼采的永恒轮回不是在避免痛苦而是让你拥抱它。克制欲望可以当训练工具，不能当人生战略。幸福密度不是被降档到\"什么都不期待\"——是敢在徒劳的球台上打完这一局。"
            ],
            "scanDate": "2026-05-27",
            "scanTarget": "723条微信读书划线 + dailyNote 原文 + 原哲学飞轮主题 + caseHelper 设计/迭代记录",
            "operatingGuide": [
                "",
                "===== 三层操作系统 =====",
                "",
                "第一层 - 战略层：游戏者模型",
                "世界是沙盒，没有标准解。设定剧本→投入演绎→获得反馈→微调算法。对过程极度认真，对结果不在乎。",
                "",
                "第二层 - 执行层：INTJ 执行力日课",
                "",
                "1. 两分钟冲动法则 — 想到一件事，两分钟内做出第一个物理动作。手比脑快是INTJ最需要练的技能。",
                "2. 把想关进笼子 — 思考截止时间到就必须开始写第一段。框架可以迭代，但只有第一段写出来迭代才可能。",
                "3. 每天早上第一件事做一件不需要想的事 — 铺床/洗杯/倒垃圾，给Te喂一口饭。",
                "4. 不可商量的最小单元 — 每天选一件事，设定不可能失败的量（写50字/挥拍10次）。",
                "5. 外部结构代替内部意志 — 把承诺说出去，激活Fi的自我一致性迫使Te跟进。",
                "6. 完成优于完美 — 贴在物理空间里，对抗Ni的还不够好。",
                "7. 识别伪努力信号 — 连续30分钟整理/分类/建框架时站起来走一圈。",
                "8. 一小时规则 — 一小时内无可见产出就切换任务，用Te的标准衡量时间。",
                "9. 吃青蛙只吃一只 — 每天精力最好时做最重要那件不想面对的事。",
                "10. 用身体活动重置大脑 — 洗冷水澡/挥拍/走路。Se是最有效的重启键。",
                "11. 晚上不决策 — 计划可以列，承诺留到早上再说。",
                "",
                "口诀：先开枪，再瞄准。你在行动中迭代，Ni应该在你行动之后出来复盘，不是之前出来阻拦。",
                "",
                "第三层 - 兜底层：失败与共处",
                "人生失败总比成功多。五层操作：承认结构（Ni看到事实）→转换焦点（Te给出方向）→指出捷径（细品失败比追寻容易）→给出回报（等来自己的那一次成功）→最后的警告（不然苟都苟不住）。此层用于Ni/Te掉线、什么都做不了的时候。"
            ],
            "recordSections": [
                {
                    "title": "原哲学飞轮归档",
                    "summary": "原先散落在哲学飞轮里的主题，不再单独维护页面，统一并到这本书里继续生长。",
                    "items": [
                        "人不退出流程，而是从执行者变成判断者、验收者和方向控制者。",
                        "从执行层升级到定义边界层——碳基到硅基的思维跃迁。",
                        "300次迭代的认知：持续做就会变好，驱动力要来自内在。",
                        "想清楚比做决定更重要。先穷尽选项空间，再聚焦最重要的。",
                        "6个潜意识模式不是缺点，是大脑的保护机制。看见它们就是手动接管。",
                        "焦急的底层燃料是对死亡和失去的恐惧。平静不是不作为，是对时间有信心。",
                        "感受阳光，闻着花香——幸福不在远方，在你能感知的当下。"
                    ]
                },
                {
                    "title": "caseHelper 设计与迭代",
                    "summary": "从手工脚本 → Agent协同 → 知识库守门员。把\"经验\"从人的脑子迁移到可被Agent加载的规则文件。",
                    "items": [
                        "上下文延迟加载：按meta.json.phase动态加载skill，不一次性灌入全部宪章。节省context budget，Agent在每个阶段只看到当前需要的规则。",
                        "子Agent编排：主Agent做阶段决策和路由，子Agent做聚焦执行。回收结构化结论，不直接写文件，避免冲突。",
                        "知识库管理员模式：探测前守门（注入api-quirks等已验证模式），探测后归档新发现。替代手工维护PlatformClient怪癖列表。",
                        "浏览器实证优先：Playwright抓Network真实Payload，API自愈机制替代curl盲猜。不在不确定的API参数上消耗推理token。",
                        "Turing门禁T1-T8：标题无时间戳、数据自包含、断言用contains、验证点标题覆盖等。样本验证→批量创建→全量调试→回填。",
                        "meta.json作为状态恢复锚点：阶段切换不需重放历史，新对话从当前phase接续。最有价值的产出不是代码，是流程认知的固化。"
                    ]
                },
                {
                    "title": "碰撞流程",
                    "summary": "以后只保留一条更短的链路，不再分叉到哲学飞轮。",
                    "items": [
                        "原文落 Note：原始观察、原话、碎片感受继续写在 dailyNote。",
                        "交互落生活随笔：经过对话、归纳、碰撞后的内容统一回到这本书。",
                        "不再往哲学飞轮写：旧页面只做存量归档或跳转，不再作为新增入口。"
                    ]
                }
            ]
        }
    },
    {
        "bookId": "3300089952",
        "title": "开始冥想",
        "coverTitle": "开始冥想",
        "author": "[法]克里斯托夫·安德烈",
        "publisher": "微信读书",
        "status": "在读",
        "progress": 10,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/23/cpplatform_wvwhaafwvh9yhkmxrkvymd/t9_cpplatform_wvwhaafwvh9yhkmxrkvymd1709628757.jpg",
        "lastReadDate": "2026-05-19",
        "readingTime": 1011,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "title": "清晰思考",
        "coverTitle": "清晰\n思考",
        "author": "Shane Parrish",
        "publisher": "中信出版社",
        "status": "在读",
        "progress": 53,
        "notes": 5,
        "edition": "决策流程",
        "theme": [
            "#e6f0f7",
            "#6a92b3"
        ],
        "source": "Obsidian 快照",
        "detail": {
            "summary": "Obsidian 已有快照与 5 个升级方向，虽然不在截图中，但已经构成书架的真实阅读来源。",
            "thesis": "人生结果取决于日常无数微小时刻中，是否能用清晰思考取代本能反应。",
            "reader": "经常在压力下做出后悔决定、想提升决策质量但不知道从哪入手的人。",
            "distance": "互补——你已经有大量感知层洞见，但缺少决策流程层的系统框架。",
            "originalFeeling": "人生结果不取决于少数几次重大决定，而取决于日常无数微小时刻中，是否能用清晰思考来取代本能反应。大多数人其实活在\"自动驾驶\"模式下。情绪默认、自负默认、社交默认、惰性默认——四种最突出也最危险的默认模式。清晰思考并不总是意味着\"更理性\"，而是要能识别出自己何时处于非理性状态，并在那一刻推迟决定。",
            "collisions": [
                "清晰思考 × 记忆资产",
                "清晰思考 × 万物相通",
                "清晰思考 × 信息不对称",
                "清晰思考 × 想清楚与做决定",
                "清晰思考 × 焦急的决策框架"
            ],
            "scanDate": "2026-04-08",
            "scanTarget": "哲学飞轮 17 张洞见卡",
            "obsidianFile": "~/Documents/codeStore/obsidian/阅读/2026-04-08-清晰思考.md"
        },
        "cover": "https://cdn.weread.qq.com/weread/cover/84/cpplatform_92xn2vnqhef5ymapkawpfg/t9_cpplatform_92xn2vnqhef5ymapkawpfg1725345567.jpg",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "bookId": "852290",
        "title": "未来简史：从智人到智神",
        "coverTitle": "未来简史：从智人到智神",
        "author": "尤瓦尔·赫拉利",
        "publisher": "中信出版集团",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "译者：林俊宏",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/44/YueWen_852290/t9_YueWen_852290.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "34560817",
        "title": "雪球专刊·特别版：段永平投资问答录商业逻辑篇",
        "coverTitle": "雪球专刊·特别版：段永平投资问答录商业逻辑篇",
        "author": "雪球专刊",
        "publisher": "浙江出版集团数字传媒有限公司",
        "status": "已读",
        "progress": 100,
        "notes": 1,
        "edition": "经济理财-理财",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书 + Obsidian",
        "cover": "https://cdn.weread.qq.com/weread/cover/44/YueWen_34560817/t9_YueWen_34560817.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "做对的事，把事情做对。投资和商业都是同一个底层逻辑——先判断是非，再谈能力，价格和时机都是次要变量。",
            "reader": "想从\"怎么选股\"升级为\"怎么判断一件事值不值得做\"的人。",
            "distance": "高度共鸣——\"本分/平常心\"和飞轮底层价值观几乎完全重合，但段永平框架更系统，洞见更个人化。",
            "originalFeeling": "",
            "collisions": [
                "段永平 × 本分与平静过滤器：物理学家与工程师"
            ],
            "scanDate": "2026-04-08",
            "scanTarget": "",
            "obsidianFile": "~/Documents/codeStore/obsidian/阅读/2026-04-08-段永平投资问答录.md"
        }
    },
    {
        "bookId": "CB_DVy1as1Y7AuO6ym6xABfb4Qw",
        "title": "每周工作4小时（新版）",
        "coverTitle": "每周工作4小时（新版）",
        "author": "【美】蒂莫西·费里斯, 鹤梅",
        "publisher": "文化发展出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_71v32B30IBEo6i26gW7MEGIk_parsecover",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_9AX0uX0tBDV36yP6xA6PfAeU",
        "title": "臣服实验 (迈克.A辛格) (Z-Library).pdf",
        "coverTitle": "臣服实验 (迈克.A辛格) (Z-Library).pdf",
        "author": "待补",
        "publisher": "微信读书",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_GBLBWJBW9G7R6jF6kfDBX4Br_parsecover",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300042157",
        "title": "注意力危机：你为什么失去专注力？如何重回深度思考？",
        "coverTitle": "注意力危机：你为什么失去专注力？如何重回深度思考？",
        "author": "约翰·海利",
        "publisher": "新华出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：董亚丽",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/49/cpPlatform_r4B98JokyMzfbwUvJemF3t/t9_cpPlatform_r4B98JokyMzfbwUvJemF3t.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300151958",
        "title": "王阳明心学入门三步走（全三册）",
        "coverTitle": "王阳明心学入门三步走（全三册）",
        "author": "度阴山",
        "publisher": "江苏凤凰文艺出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "哲学宗教-东方哲学",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/76/cpplatform_nkcjju6fvrdgeurscrqqr5/t9_cpplatform_nkcjju6fvrdgeurscrqqr51752030599.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_9VG9yA9yP0Um6xI6xA4RY8aW",
        "title": "鱼不存在 - 露露·米勒",
        "coverTitle": "鱼不存在 - 露露·米勒",
        "author": "【美】露露·米勒",
        "publisher": "湖南文艺出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_AVv8Rv8QV3w66rm6st3kN4tn_parsecover",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300048761",
        "title": "鱼不存在",
        "coverTitle": "鱼不存在",
        "author": "露露·米勒",
        "publisher": "湖南文艺出版社",
        "status": "在读",
        "progress": 9,
        "notes": 0,
        "edition": "译者：陈晓宇",
        "theme": [
            "#335db6",
            "#1f2f6a"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/75/cpplatform_cvbkmm6ahiy3dkkjm6cbeu/t9_cpplatform_cvbkmm6ahiy3dkkjm6cbeu1676431298.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "837932",
        "title": "穷查理宝典：查理·芒格智慧箴言录（全新增订本）",
        "coverTitle": "穷查理宝典：查理·芒格智慧箴言录（全新增订本）",
        "author": "彼得·考夫曼",
        "publisher": "中信出版集团",
        "status": "在读",
        "progress": 17,
        "notes": 0,
        "edition": "译者：李继宏 等",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/48/YueWen_837932/t9_YueWen_837932.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300054712",
        "title": "三国争霸（全三册）",
        "coverTitle": "三国争霸（全三册）",
        "author": "渤海小吏",
        "publisher": "中国大百科全书出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "历史-中国古代",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/20/cpplatform_n2g8fnd9eu4fj5fssec8sy/t9_cpplatform_n2g8fnd9eu4fj5fssec8sy1680848708.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300070929",
        "title": "埃隆·马斯克传",
        "coverTitle": "埃隆·马斯克传",
        "author": "沃尔特·艾萨克森",
        "publisher": "中信出版集团",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "译者：孙思远 刘家琦",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/37/cpplatform_gmaqcuuuamjkvu7knomjim/t9_cpplatform_gmaqcuuuamjkvu7knomjim1703077934.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "22664120",
        "title": "为什么精英都是时间控",
        "coverTitle": "为什么\n精英都是\n时间控",
        "author": "桦泽紫苑",
        "publisher": "湖南文艺出版社",
        "status": "已读",
        "progress": 100,
        "notes": 1,
        "edition": "时间管理",
        "theme": [
            "#7fc8e9",
            "#d9f0f7"
        ],
        "source": "截图 + 旧读书笔记 + 微信读书",
        "cover": "https://cdn.weread.qq.com/weread/cover/11/YueWen_22664120/t9_YueWen_22664120.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "旧博客已有读书笔记，当前书架先把它作为待迁移条目。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [
                "时间管理相关旧笔记待迁入 Obsidian 快照"
            ],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "25615383",
        "title": "涛动周期论：经济周期决定人生财富命运",
        "coverTitle": "涛动周期论：经济周期决定人生财富命运",
        "author": "周金涛等编著",
        "publisher": "机械工业出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "经济理财-财经",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/71/YueWen_25615383/t9_YueWen_25615383.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "CB_Bku1ZA1Y7FQC6dA6cR",
        "title": "送你一颗子弹",
        "coverTitle": "送你一颗子弹",
        "author": "刘瑜",
        "publisher": "epub掌上书苑",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "微信读书",
        "theme": [
            "#f6f9f4",
            "#8ac39c"
        ],
        "source": "微信读书同步",
        "cover": "https://res.weread.qq.com/wrepub/CB_Bku1ZA1Y7FQC6dA6cR_parsecover",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "34389621",
        "title": "少有人走的路（1-8套装）",
        "coverTitle": "少有人走的路（1-8套装）",
        "author": "M.斯科特·派克",
        "publisher": "北京联合出版公司",
        "status": "在读",
        "progress": 22,
        "notes": 0,
        "edition": "译者：严冬冬,张然,李荣等",
        "theme": [
            "#f4ebd5",
            "#d9c18d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/58/yuewen_34389621/t9_yuewen_343896211678699158.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "3300016646",
        "title": "可能性的艺术：比较政治学30讲",
        "coverTitle": "可能性的艺术：比较政治学30讲",
        "author": "刘瑜",
        "publisher": "广西师范大学出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "政治军事-政治",
        "theme": [
            "#fff3df",
            "#e6ad52"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/46/3300016646/t9_3300016646.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "bookId": "23741950",
        "title": "吉他自学一本通：120首弹唱曲超精选",
        "coverTitle": "吉他自学一本通：120首弹唱曲超精选",
        "author": "唐文通",
        "publisher": "化学工业出版社",
        "status": "未读",
        "progress": 0,
        "notes": 0,
        "edition": "艺术-音乐",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "微信读书同步",
        "cover": "https://cdn.weread.qq.com/weread/cover/1/YueWen_23741950/t9_YueWen_23741950.jpg",
        "lastReadDate": "",
        "readingTime": 0,
        "detail": {
            "summary": "微信读书已同步，读书快照待补。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "originalFeeling": "",
            "collisions": [],
            "scanDate": "",
            "scanTarget": ""
        }
    },
    {
        "title": "窄门",
        "coverTitle": "窄门",
        "author": "安德烈·纪德",
        "publisher": "果麦经典",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "果麦经典",
        "theme": [
            "#f2e8d9",
            "#d7cab5"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "当前只在书架层收录，读书快照还没从 Obsidian 建立。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "title": "金钱心理学",
        "coverTitle": "金钱心理学",
        "author": "摩根·豪泽尔",
        "publisher": "中信出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "Psychology of Money",
        "theme": [
            "#fff3e6",
            "#eb7a4d"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "已经在书架层收录，后续适合补成“财富叙事 / 人性偏差”方向的读书快照。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "title": "悉达多",
        "coverTitle": "悉达多",
        "author": "赫尔曼·黑塞",
        "publisher": "果麦经典",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "果麦经典",
        "theme": [
            "#98806b",
            "#dccba6"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "当前只放书架卡，后续可补“修行 / 经历 / 自我抵达”方向的快照。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "title": "这个哲学家救了我",
        "coverTitle": "这个哲学家\n救了我",
        "author": "马西莫·匹格里奇",
        "publisher": "北京联合出版公司",
        "status": "在读",
        "progress": 86,
        "notes": 0,
        "edition": "斯多葛人生生存术",
        "theme": [
            "#5e88c8",
            "#9db9de"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "当前只放书架卡，适合后续补哲学疗愈方向的快照。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "title": "曾国藩传",
        "coverTitle": "曾国藩传",
        "author": "张宏杰",
        "publisher": "岳麓书社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "晚清人物",
        "theme": [
            "#131313",
            "#7f694f"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "当前只在书架层收录，适合后续补人物传记方向的读书卡。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "title": "人类简史",
        "coverTitle": "人类简史",
        "author": "尤瓦尔·赫拉利",
        "publisher": "中信出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "从动物到上帝",
        "theme": [
            "#d7edf4",
            "#95b8c8"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "当前只在书架卡层收录。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    },
    {
        "title": "崔玉涛自然养育法",
        "coverTitle": "自然\n养育法",
        "author": "崔玉涛",
        "publisher": "中信出版社",
        "status": "已读",
        "progress": 100,
        "notes": 0,
        "edition": "养育实践",
        "theme": [
            "#ffefb7",
            "#8fb8e8"
        ],
        "source": "截图整理",
        "detail": {
            "summary": "当前只在书架卡层收录。",
            "thesis": "待补",
            "reader": "待补",
            "distance": "待补",
            "collisions": []
        },
        "cover": "",
        "lastReadDate": "",
        "readingTime": 0
    }
];
